...
public class day4월3일7 {
	...
	public static void main(String[] args) {
		...
		if(createRandArr(min, max, arr2)){
		 ...
		 // 2등 : 보너스번호를 제외한 번호 6개가 일치
		 // 3등 : 보너스번호를 제외한 번호 6개가 일치
		 // 4등 : 보너스번호를 제외한 번호 6개가 일치 
		 // 5등 : 보너스번호를 제외한 번호 6개가 일치
		//   꽝: 2개이하
	...
	public static boolean createRandArr(int min, int max, int []arr){
		...
		//생성할 수 잇는 숫자의 갯수보다 배열의 크기가 클때
		//중북되는 상황이 발생할 수밖에 없을때
		...
		while(cnt < arr.length){
			...
			//arr에 r이 없으면
			if(!contain(arr, r, cnt)){//cnt쓴이유{괄호안에}arr.length보다 비교횟수가 적기때문에
				arr[cnt] = r;
				cnt++;
			}
		}
	...
	public static boolean createRandArr2(int min, int max, int []arr){
	   ...
	   //매개변수 : 두 배열 => ㅑㅜㅅ []arr1,intp[]arr2
	   //리턴타입 : 갯수 =>int
	   //매소드명 : compareArr
	   
	 
	public static int compareArr(int[]arr1, int []arr2){
		int cnt = 0; // 두배여의 원소 중 일치하는 갯수
